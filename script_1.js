//this is home page javascript
// here I make a favourites key for storing all favourites hero's id in local storage if not available
if (localStorage.getItem("favourites")==null) {
  ...
  // this is the function for show heros full details in a new page
  function showDetails(idnumber) {
    localStorage.setItem("id", idnumber);
    window.location = "index_2.html";
  }
    
  // this is the function for adding id value in local storage favourites key if not available this id 
  function addFavourite(id) {
    if (!arr.includes(id) == true) {
      arr.push(id);
      localStorage.setItem("favourites", JSON.stringify(arr));
      alert("Hurry!! your hero added in favourites")
    }else{
      alert("Oops!,your hero already added in favourites")
    }
  }
  
  
  //this is the function for show heros depends on search also filter heros depends on key press
